...
function Samples() {
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            let rsc = 'samples?program=' + prog+'&page='+page
            fetchSamples(rsc)
        }
                ...
                <div className="row">
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-center">
                        <div className="d-flex w-100 flex-row flex-wrap justify-content-center justify-content-lg-between align-items-center gap-lg-4">
                            ...
                            <ProgramsNavbar program={programData} router={router} subtitle="Samples" />
                            <Link href="/admin/programs/[program]/samples/new" as={`/admin/programs/${prog}/samples/new`}>
                                ...
                                <table className="table table-striped table-hover table-bordered">
                                    ...
                                    <tbody>
                                        {samples && samples.length > 0 ? samples.map((sample) => (
                                            ...
                                            <tr key={sample.uuid}>
                                                <td>
                                                    <Link href={`/admin/programs/${prog}/samples/${sample.uuid}`}>
                                                        <a className="">{sample.name}</a>
                                                    </Link>
                                                </td>
                                                <td>
                                                    {sample?.description}
                                                </td>
                                                <td className='text-capitalize'>{sample.deleted_at == null ? <span className='badge bg-success'>Active</span> : <span className='badge bg-warning'>Disabled</span>}</td>
                                                <td>{new Date(sample?.created_at).toLocaleString('en-GB', {
                                                    year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true
                                                }) || "-"}</td>
                                                <td className="d-flex flex-column flex-lg-row gap-2 justify-content-center">
                                                    <Link href={{ pathname: `/admin/programs/${prog}/samples/${sample.uuid}/edit` }} >
                                                    ...
                                                    <a className='btn text-danger btn-link btn-sm py-0 text-nowrap' onClick={ev => {
                                                        ...
                                                        if (confirm('Are you sure you want to delete this sample?')) {
                                                            getResource(`sample/delete/${sample.uuid}`, { method: 'DELETE' }).then((data) => {
                                                                if (data.status === true) {
                                                                    setStatus('success')
                                                                    setMessage('Sample deleted successfully')
                                                                    setSamples(samples.filter(u => u.uuid !== sample.uuid))
                                                                } else {
                                                                    setStatus('error')
                                                                    setMessage(data.message)
                                                                }
                                                                ...
                                                                console.log(err)
                                                                setStatus('error')
                                                                setMessage('Error deleting sample: ' + err.message || err)
                                                            })
                                                        }
                                                    }}>Delete</a>
                                                </td>
                                            </tr>
                                        )) : <tr><td colSpan={7} className="text-center">No samples found</td></tr>}
                                    </tbody>
                                </table>
                            </div>
                        </div>
