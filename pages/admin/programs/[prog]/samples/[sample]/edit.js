...
function EditSample() {
    ...
    const { sample, prog } = router.query
    ...
    const saveSample = () => {
        setLoading(true)
        getResource('sample/edit/' + sample, { method: 'PUT', body: newSampleData }).then((data) => {
            if (data.status === true) {
                setStatus('success')
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            getResource(`sample/${sample}`).then((data) => {
                if (data.status === true) {
                    setNewSampleData({
                        "name": data.data.name || "",
                        ...
                        "expected_outcome": data.data.expected_outcome || "",
                        "expected_outcome_notes": data.data.expected_outcome_notes || "",
                        "expected_interpretation": data.data.expected_interpretation || "",
                        "expected_interpretation_notes": data.data.expected_interpretation_notes || "",
                    })
                } else {
                    setStatus('error')
                    setMessage(data.message)
                ...
                console.log(err)
                setStatus('error')
                setMessage('Error loading sample: ' + err.message || err)
                setLoading(false)
            })
        }
                ...
                <div className="row mb-4 ">
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-center">
                        <div className="d-flex w-100 flex-row flex-wrap justify-content-center justify-content-lg-between align-items-center gap-lg-4">
                            ...
                            <h3>Edit sample</h3>
                            <Link href={`/admin/programs/${prog}/samples/${sample}`}>
                    ...
                    <div className='col-lg-12 my-3'>
                        {status !== 'error' && <form onSubmit={ev => {
                            ...
                            <div className="w-100 d-flex align-items-center justify-content-center">
                                <button type="submit" className="btn btn-primary">Update sample</button>
                            </div>
                        </form>}
                    </div>
                </div>
