...
function NewPanel() {
    ...
    const savePanel = () => {
        setLoading(true)
        getResource('panel/new', { method: 'POST', body: newPanelData }).then((data) => {
            if (data.status === true) {
                setStatus('success')
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            setLoading(false)
            if (prog) {
                setNewPanelData({ ...newPanelData, program: prog })
            }
        }
                    ...
                    <div className='col-lg-12 my-3'>
                        {status !== 'error' && <form onSubmit={ev => {
                            ...
                            <div className="w-100 d-flex align-items-center justify-content-center">
                                <button type="submit" className="btn btn-primary">Save panel</button>
