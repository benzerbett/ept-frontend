...
function EditRound() {
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            getResource(`round/${round}`).then((data) => {
                if (data.status === true) {
                    setNewRoundData({
                        "name": data.data.name || "",
                        ...
                        "active": (data.data.active == 1 || data.data.active == true) || false,
                        "testing_instructions": data.data.testing_instructions || "",
                        "start_date": data.data.start_date ? new Date(data.data.start_date).toISOString().slice(0, 10) : "",
                        "end_date": data.data.end_date ? new Date(data.data.end_date).toISOString().slice(0, 10) : "",
                        "meta": data.data.meta || "",
                    })
                } else {
                    setStatus('error')
                    ...
                    <div className='col-lg-12 my-3'>
                        {status !== 'error' && <form onSubmit={ev => {
                            ...
                            <div className="w-100 d-flex align-items-center justify-content-center">
                                <button type="submit" className="btn btn-primary">Update round</button>
                            </div>
                        </form>}
                    </div>
                </div>
