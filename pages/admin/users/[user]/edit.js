...
function EditUser() {
    ...
    const saveUser = () => {
        ...
        setLoading(true)
        getResource('user/edit/'+user, { method: 'PUT', body: userData }).then((data) => {
            if (data.status === true) {
                setStatus('success')
            ...
            console.log(err)
            setStatus('error')
            setMessage('Error updating user: ' + err.message || err)
            setLoading(false)
        })
    }
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            setMessage('')
            setLoading(false)
            getResource(`user/${user}`).then((data) => {
                if (data.status === true) {
                    setUserData({
                        "name": data.user.name || "",
                        "email": data.user.email || "",
                        "role": data.user.role || "",
                        "programs": (data.programs && data.programs.length > 0) ? Array.from(data.programs, pg=>pg.uuid) : [],
                        "meta": data.user.meta || ""
                    })
                } else {
                    setStatus('error')
                ...
                console.log(err)
                setStatus('error')
                setMessage('Error fetching user details: ' + err.message || err)
                setLoading(false)
            })
            fetchRoles()
            fetchPrograms()
    ...
    return (
        <>
            <Head>
                <title>EPT | Edit User Account</title>
                    ...
                    <div className='col-lg-12 my-3'>
                        <form onSubmit={ev => {
                            ...
                            <div className="form-group row mb-2 mb-lg-3">
                                ...
                                <div className='col-lg-8'>
                                    <select className='form-select' value={userData.role} onChange={ev => {
                            ...
                            {(userData.role && userData.role !== roles.find(rl => rl.name == 'guest')?.uuid) &&
                                <div className="form-group row mb-2 mb-lg-3">
                                    ...
                                    <div className='col-lg-8'>
                                        <select multiple className='form-select' value={userData.programs} onChange={ev => {
